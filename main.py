...
# CONFIG
RELEASE_DATE = date(2026, 1, 8)
TARGET_MOVIE_ID = 243312
...
if __name__ == "__main__":
    ...
    for entry in all_data:
        ...
        if not show_date:
            continue  # skip bad record safely

        show_key = (
            entry.get("theater_name", "UNKNOWN"),
            show_date,
        ...
        format_name = entry.get("format", "standard").lower()

        if format_name not in grouped[show_key]["formats"]:
            grouped[show_key]["formats"][format_name] = {
                "format": format_name,
                "totalSeatSold": 0,
                "totalSeatCount": 0,
                "grossRevenueUSD": 0.0,
        ...
        fmt = grouped[show_key]["formats"][format_name]
    ...
    for show in grouped.values():
        ...
        has_dbox = "d-box" in formats
        has_standard = "standard" in formats

        # 👉 Skip decrease if D-BOX is 100% full
        if has_dbox and has_standard:
            dbox = formats["d-box"]
            standard = formats["standard"]

            dbox_occupancy = (
                (dbox["totalSeatSold"] / dbox["totalSeatCount"]) * 100
                if dbox["totalSeatCount"] > 0 else 0
            )

            if dbox_occupancy < 100:
                decrease_seats = min(
                    dbox["totalSeatSold"],
                ...
                decrease_amount = (
                    decrease_seats * standard["adultTicketPrice"]
                )

                standard["grossRevenueUSD"] = round(
                    max(0.0, standard["grossRevenueUSD"] - decrease_amount),
                    2
                )
        ...
        for fmt in formats.values():
            if fmt["totalSeatCount"] > 0:
                fmt["occupancy"] = round(
                    (fmt["totalSeatSold"] / fmt["totalSeatCount"]) * 100, 2
                )
            else:
                fmt["occupancy"] = 0.0
    ...
    print(f"🎯 TOTAL GROSS (ALL SHOWS): ${total_gross}")
